...
AddEventHandler('esx:playerLoaded', function(xPlayer)
	...
	local idTpl = '<div>MajorkaRP: &nbsp;{{id_label}}<img src="img/accounts/id.png"/></div>'
		
		ESX.UI.HUD.RegisterElement('myId', #xPlayer.accounts, 0, idTpl, {
			id_label = 0
		})  
		
		ESX.UI.HUD.UpdateElement('myId', {
...
AddEventHandler('esx:removeInventoryItem', function(item, count)
	...
	if item.name ~= 'pistol_ammo' and item.name ~= 'smg_ammo' and item.name ~= 'shotgun_ammo' and item.name ~= 'flare_ammo' and item.name ~= 'rifle_ammo' and item.name ~= 'sniper_ammo' and item.name ~= 'pocisk' then
