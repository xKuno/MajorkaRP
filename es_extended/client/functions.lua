...
ESX.Game.GetClosestObject = function(filter, coords)
  ...
  for _, object in ipairs(ESX.Game.GetObjects()) do
    if scope(object) then
      local distance = #(GetEntityCoords(object, true) - coords)
...
ESX.Game.GetPlayersInArea = function(coords, area)
  ...
  for _, player in ipairs(ESX.Game.GetPlayers()) do
    local target = GetPlayerPed(player)
    if #(GetEntityCoords(target, true) - coords) <= area then
...
ESX.Game.GetClosestVehicle = function(coords)
  ...
  for _, vehicle in ipairs(ESX.Game.GetVehicles()) do
    local distance = #(GetEntityCoords(vehicle, true) - coords)
    if closestDistance == -1 or closestDistance > distance then
...
ESX.Game.GetVehiclesInArea = function(coords, area)
  ...
  for _, vehicle in ipairs(ESX.Game.GetVehicles()) do
    if #(GetEntityCoords(vehicle, true) - coords) <= area then
...
ESX.Game.IsSpawnPointClear = function(coords, radius)
  coords = vec3(coords.x, coords.y, coords.z)
  for _, vehicle in ipairs(ESX.Game.GetVehicles()) do
    if #(GetEntityCoords(vehicle, true) - coords) <= radius then
      return false
    end
  end
...
ESX.Game.GetClosestPed = function(coords, ignoreList)
  ...
  coords = vec3(coords.x, coords.y, coords.z)
  for _, ped in ipairs(ESX.Game.GetPeds(ignoreList)) do
...
ESX.Game.Utils.DrawText3D = function(coords, text, size)
  ...
  local dist = #(vec3(camCoords.x, camCoords.y, camCoords.z) - vec3(coords.x, coords.y, coords.z))
...
ESX.ShowInventory = function()
	...
	table.insert(elements, {
		label     = '<span style="color:lightblue;">'.."--- Praca ---"..'</span>'
	})
	  ...
	  table.insert(elements, {
			label     = 'Organizacja: ' .. ESX.PlayerData.job2.label .. ' - ' .. ESX.PlayerData.job2.grade_label
	  })


	table.insert(elements, {
		label     = '<span style="color:lightblue;">'.."--- Pieniadze ---"..'</span>'
	})
	...
	table.insert(elements, {
		label     = '<span style="color:lightblue;">'.."--- Itemy ---"..'</span>'
	})

	for i=1, #ESX.PlayerData.inventory, 1 do
			if ESX.PlayerData.inventory[i].count > 0 and (ESX.PlayerData.inventory[i].name ~= 'pistol'  and ESX.PlayerData.inventory[i].name ~= 'snspistol' and ESX.PlayerData.inventory[i].name ~= 'doubleaction' and ESX.PlayerData.inventory[i].name ~= 'combatpistol' and ESX.PlayerData.inventory[i].name ~= 'vintagepistol' and ESX.PlayerData.inventory[i].name ~= 'pistol_mk2' and ESX.PlayerData.inventory[i].name ~= 'snspistol_mk2' and ESX.PlayerData.inventory[i].name ~= 'revolver' and ESX.PlayerData.inventory[i].name ~= 'pistol50' and ESX.PlayerData.inventory[i].name ~= 'heavypistol'
			and ESX.PlayerData.inventory[i].name ~= 'knife'   and ESX.PlayerData.inventory[i].name ~= 'knuckle'  and ESX.PlayerData.inventory[i].name ~= 'nightstick'  and ESX.PlayerData.inventory[i].name ~= 'hammer'  and ESX.PlayerData.inventory[i].name ~= 'bat'  and ESX.PlayerData.inventory[i].name ~= 'golfclub' and ESX.PlayerData.inventory[i].name ~= 'crowbar' and ESX.PlayerData.inventory[i].name ~= 'bottle' and ESX.PlayerData.inventory[i].name ~= 'dagger' and ESX.PlayerData.inventory[i].name ~= 'hatchet' and ESX.PlayerData.inventory[i].name ~= 'machete'
			and ESX.PlayerData.inventory[i].name ~= 'flashlight' and ESX.PlayerData.inventory[i].name ~= 'switchblade' and ESX.PlayerData.inventory[i].name ~= 'poolcue' and ESX.PlayerData.inventory[i].name ~= 'pipewrench' and ESX.PlayerData.inventory[i].name ~= 'ball' and ESX.PlayerData.inventory[i].name ~= 'stungun' and ESX.PlayerData.inventory[i].name ~= 'molotov' and ESX.PlayerData.inventory[i].name ~= 'smg_mk2' and ESX.PlayerData.inventory[i].name ~= 'machinepistol' and ESX.PlayerData.inventory[i].name ~= 'pumpshotgun' and ESX.PlayerData.inventory[i].name ~= 'carbinerifle'
			and ESX.PlayerData.inventory[i].name ~= 'smg' and ESX.PlayerData.inventory[i].name ~= 'microsmg' and ESX.PlayerData.inventory[i].name ~= 'handcuffs')then 
			table.insert(elements, {
		...
		table.insert(elements, {
			label     = '<span style="color:lightblue;">'.."--- Bron ---"..'</span>'
		})

	for i=1, #ESX.PlayerData.inventory, 1 do
		if ESX.PlayerData.inventory[i].count > 0 and  (ESX.PlayerData.inventory[i].name == 'pistol'  or ESX.PlayerData.inventory[i].name == 'snspistol' or ESX.PlayerData.inventory[i].name == 'combatpistol' or ESX.PlayerData.inventory[i].name == 'doubleaction' or ESX.PlayerData.inventory[i].name == 'vintagepistol' or ESX.PlayerData.inventory[i].name == 'pistol_mk2' or ESX.PlayerData.inventory[i].name == 'snspistol_mk2' or ESX.PlayerData.inventory[i].name == 'revolver' or ESX.PlayerData.inventory[i].name == 'pistol50' or ESX.PlayerData.inventory[i].name == 'heavypistol'
		or ESX.PlayerData.inventory[i].name == 'knife'   or ESX.PlayerData.inventory[i].name == 'knuckle'  or ESX.PlayerData.inventory[i].name == 'nightstick'  or ESX.PlayerData.inventory[i].name == 'hammer'  or ESX.PlayerData.inventory[i].name == 'bat'  or ESX.PlayerData.inventory[i].name == 'golfclub' or ESX.PlayerData.inventory[i].name == 'crowbar' or ESX.PlayerData.inventory[i].name == 'bottle' or ESX.PlayerData.inventory[i].name == 'dagger' or ESX.PlayerData.inventory[i].name == 'hatchet' or ESX.PlayerData.inventory[i].name == 'machete'
		or ESX.PlayerData.inventory[i].name == 'flashlight' or ESX.PlayerData.inventory[i].name == 'switchblade' or ESX.PlayerData.inventory[i].name == 'poolcue' or ESX.PlayerData.inventory[i].name == 'pipewrench' or ESX.PlayerData.inventory[i].name == 'ball' or ESX.PlayerData.inventory[i].name == 'stungun' or ESX.PlayerData.inventory[i].name == 'molotov' or ESX.PlayerData.inventory[i].name == 'smg_mk2' or ESX.PlayerData.inventory[i].name == 'smg' or ESX.PlayerData.inventory[i].name == 'microsmg'
		or ESX.PlayerData.inventory[i].name == 'machinepistol' or ESX.PlayerData.inventory[i].name == 'pumpshotgun' or ESX.PlayerData.inventory[i].name == 'carbinerifle' or ESX.PlayerData.inventory[i].name == 'handcuffs') then 		
			table.insert(elements, {
				label     = ESX.PlayerData.inventory[i].label .. ' x' .. ESX.PlayerData.inventory[i].count,
				count     = ESX.PlayerData.inventory[i].count,
				type      = 'item_standard',
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'inventory',
	{
		...
		if data.current.usable then
			table.insert(elements, {label = _U('use'), action = 'use', type = data.current.type, value = data.current.value, itemlabel = data.current.itemlabel})
		end

		if data.current.canRemove then
			if player ~= -1 and distance <= 3.0 then
				table.insert(elements, {label = _U('give'), action = 'give', type = data.current.type, value = data.current.value, itemlabel = data.current.itemlabel})
			end

			table.insert(elements, {label = _U('remove'), action = 'remove', type = data.current.type, value = data.current.value, itemlabel = data.current.itemlabel})
		end
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'inventory_item',
		{
			...
			local itemlabel = data1.current.itemlabel
			...
			if data1.current.action == 'give' then
				...
				ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'give_item_to',
				{
					...
					if type == 'item_weapon' then
						...
						TriggerServerEvent('3dme:shareDisplay', 'Obywatel '..GetPlayerServerId(PlayerId()) ..' daje broń z ' .. sourceAmmo ..'Nabojami Obywatelowi '..GetPlayerServerId(nearbyPlayer))
						TriggerServerEvent('esx:giveInventoryItem', GetPlayerServerId(nearbyPlayer), type, item, sourceAmmo,itemlabel)
						...
						ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'inventory_item_count_give', {
							...
							if quantity ~= nil then
								if type == 'item_standard' then
									TriggerServerEvent('3dme:shareDisplay', 'Obywatel o ID '..GetPlayerServerId(PlayerId()) ..' daje przedmiot '..itemlabel.. ' w ilości ' ..quantity.. ' sztuk,  Obywatelowi o ID '..GetPlayerServerId(nearbyPlayer))
									TriggerServerEvent('esx:giveInventoryItem', GetPlayerServerId(nearbyPlayer), type, item, quantity,itemlabel)
								elseif type == 'item_account' or type == 'item_money' then
									TriggerServerEvent('3dme:shareDisplay', 'Obywatel o ID '..GetPlayerServerId(PlayerId()) ..' daje plik banknotów Obywatelowi o ID'..GetPlayerServerId(nearbyPlayer))
									TriggerServerEvent('esx:giveInventoryItem', GetPlayerServerId(nearbyPlayer), type, item, quantity,itemlabel)
								end
								menu3.close()
								menu2.close()
			...
			elseif data1.current.action == 'remove' then
				...
				if type == 'item_weapon' then
					...
					TriggerServerEvent('3dme:shareDisplay', 'Obywatel o ID: '.. GetPlayerServerId(PlayerId()) ..' wyrzuca broń')
					TriggerServerEvent('esx:removeInventoryItem', type, item, nil, itemlabel)
					...
					ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'inventory_item_count_remove', {
						...
						if quantity == nil then
							...
							if type == 'item_standard' then
								TriggerServerEvent('esx:removeInventoryItem', type, item, quantity, itemlabel)
								TriggerServerEvent('3dme:shareDisplay', 'Obywatel '.. GetPlayerServerId(PlayerId()) .. ' wyrzuca x' .. quantity .. ' ' .. itemlabel)
								elseif type == 'item_account' or type == 'item_money' then
								TriggerServerEvent('esx:removeInventoryItem', type, item, quantity, itemlabel)
								TriggerServerEvent('3dme:shareDisplay', 'Obywatel '.. GetPlayerServerId(PlayerId()) .. ' wyrzuca x' .. quantity .. ' banknotów')
							end
							menu2.close()
							menu1.close()
						end
			...
			elseif data1.current.action == 'use' then
				TriggerServerEvent('esx:useItem', item, itemlabel)
