...
function DeleteJobVehicle()
	local playerPed = PlayerPedId()
		...
		if IsInAuthorizedVehicle() and wyciagnal == true then
			...
			ESX.ShowNotification('Tylko Samochody Winiarza Mogą tu Parkować/Juz odlozyles samochod!')
		end
end
...
if onDuty == true then
	if not ESX.Game.IsSpawnPointClear(Config.Zones.Job1.Pos, 14.0) then
		TriggerEvent("pNotify:SendNotification", {text = "Ktoś pozostawił pojazd na terenie sadu, ZBIORY WSTRZYMANE!"})
		Citizen.Wait(2000)
		HasAlreadyEnteredMarker, LastZone = true, currentZone
		TriggerEvent('esx_vigne:hasEnteredMarker', currentZone)
	else
	if IsPedInAnyVehicle(PlayerPedId()) then
		...
		if tree ~= 0 then
			...
			ESX.ShowHelpNotification("~w~Podejdź do auta i wciśnij ~g~E ~w~aby odlożyć owoce")
			Citizen.Wait(3000)
			HasAlreadyEnteredMarker, LastZone = true, currentZone
			TriggerEvent('esx_vigne:hasEnteredMarker', currentZone)
			end
...
if onDuty == true then
	...
	local tree = dupa()
		...
		ESX.ShowHelpNotification("~w~Podejdź do auta i wciśnij ~g~E ~w~aby odlożyć butelki")
		Citizen.Wait(3000)
		HasAlreadyEnteredMarker, LastZone = true, currentZone
		TriggerEvent('esx_vigne:hasEnteredMarker', currentZone)
		end
		else
		ESX.ShowHelpNotification('Musisz podejść bliżej drzewa aby coś z niego zebrać22.')
		Citizen.Wait(2000)
		HasAlreadyEnteredMarker, LastZone = true, currentZone
		TriggerEvent('esx_vigne:hasEnteredMarker', currentZone)
				end
...
if not BlipsAdded then
  ...
  BeginTextCommandSetBlipName("STRING")
  AddTextComponentSubstringPlayerName('Sprzedaż winogron')
