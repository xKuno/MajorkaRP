  ...
  while true do
    ...
    Citizen.Wait(0)
	    ...
	    if IsControlJustPressed(1, Keys["X"]) then
			...
			if #(vector3(x,y,z) - GetEntityCoords(playerPed)) > 5 and x ~= nil then
					stopEvent()
			end
		end
  end
...
RegisterNetEvent('sandy_dragi:setplayerdrugs')
AddEventHandler('sandy_dragi:setplayerdrugs', function(coords)
	Narko = coords
	allowmarker = true
	Wait(1000)
	print(coords)
...
AddEventHandler("esx_receptury:RequestStart", function(item_name, time, location)
  currentItem = options[item_name]
	currentActionTime = time
  isInLocation = location
	local playerPed = GetPlayerPed(-1)
	local chuj = item_name
	print(chuj)
	TriggerEvent("izone:isPlayerInAtLeastInOneZoneInCat", chuj, function(isIn)
		isInLocation = isIn
		print(isIn)
		if isInLocation then
			TriggerEvent("izone:isPlayerInZone", 'spalone', function(isIn)
				kutas = isIn
				if kutas == true then
					isInLocation = false
				else
  ...
  for k, Cords in pairs(currentItem.cords) do
      if IsPedInAnyVehicle(playerPed) then
				exports.pNotify:SendNotification({text = _U('vehicle_deny')})
        do return end
      end
			if isActionStarted then
				
				exports.pNotify:SendNotification({text = _U('wait_end')})
        do return end
			end
      if isInLocation == true then
					isActionStarted = true
					ESX.UI.Menu.CloseAll()
          TriggerServerEvent('esx_receptury:RemoveItem', item_name)
					exports.pNotify:SendNotification({text = currentItem.start_msg})
  ...
  if not isInLocation then
    exports.pNotify:SendNotification({text = _U('wrong_place')})
  else
...
AddEventHandler("esx_receptury:StartAction", function(currentStep, currentAction)
  Citizen.CreateThread(function()
	  ...
	  if IsControlJustPressed(1, Keys["X"]) then
	      ...
	      if data.current.value == currentAction.correct then
	        ...
	        --Show failed object, when player select wrong option
					...
					local playerPed = GetPlayerPed(-1)
					FreezeEntityPosition(playerPed, false)
					exports.pNotify:SendNotification({text = currentItem.fail_msg,})
	        spawnEndObject(currentItem.object, currentItem.end_object, x, y, z)
					TriggerEvent('esx_receptury:storeStep', currentStep)
	      end
			end
   ...
   --Function when player 'close' ui, but i realy dont even know how it work, bcs he work also when player select option...
	 function(data, menu)
		exports.pNotify:SendNotification({text = _U('doing_action')})
		exports.pNotify:SendNotification({text = _U('how_cancel')})
...
function stopEvent()
	...
	x = nil
	isActionStarted = false
	exports.pNotify:SendNotification({text = _U('cancel')})
	exports.pNotify:SendNotification({text = currentItem.fail_msg})
end
