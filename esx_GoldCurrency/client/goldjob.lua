...
ESX = nil
local timing, isPlayerWhitelisted = math.ceil(1 * 80000), false
...
AddEventHandler('esx_goldCurrency:outlawNotify', function(alert)
	if isPlayerWhitelisted then
		TriggerEvent('chat:addMessage', { args = { "^5 ALARM: " .. alert }})
	end
end)
...
AddEventHandler("esx_goldCurrency:startMission",function(spot)
	...
	if numy == 100 then
		ESX.ShowNotification("Brak ~y~dostepnych prac ~s~ w tym momencie, prosze ~b~sprobowac ponownie~s~ pozniej!")
	else
		CurrentMission = num
...
AddEventHandler("esx_goldCurrency:missionComplete", function(itemAmount1,item1,itemAmount2,item2)
    ...
    AnnounceHeader = "~g~Misja Wykonana"
	if itemAmount2 or item2 then
		AnnounceString = "+Otrzymales: ~b~"..itemAmount1.."x~s~ ~y~"..item1.."~s~ and "..itemAmount2.."x~s~ ~y~"..item2.."~s~"
	else
		AnnounceString = "+Otrzymales: ~b~"..itemAmount1.."x~s~ ~y~"..item1.."~s~"
	end
    Citizen.Wait(5 * 1000)
...
AddEventHandler("esx_goldCurrency:missionFailDeath", function()
	SetAudioFlag("LoadMPData", true)
    PlaySoundFrontend(-1, "RACE_PLACED", "HUD_AWARDS", 1)
    AnnounceHeader = "~r~Misja zakonczona Niepowodzeniem"
    AnnounceString = "Zginales"
    Citizen.Wait(5 * 1000)
    AnnounceString = false
end)
...
Citizen.CreateThread(function()
	while true do
		...
		for k,v in pairs(Config.MissionNPC) do
			local distance = #(GetEntityCoords(GetPlayerPed(-1)) - vector3(pos.x, pos.y, pos.z))
			if distance <= 1.5 and (JobInProgress == false and talkingWithNPC == false) then
				DrawText3Ds(pos.x, pos.y, pos.z, "Nacisnij ~g~[E]~s~ aby ~y~Porozmawiac~s~")
...
function requestMissionFromNPC()
	...
	ESX.TriggerServerCallback('esx_goldCurrency:getGoldJobCoolDown', function(cooldownTimer)
		...
		if not cooldownTimer then
			...
			exports['MajorkaRP']:startUI((9.5 * 1000), "Rozmowa")
...
AddEventHandler('esx_goldCurrency:startTheEvent', function(num)
	...
	while not JobCompleted and not StopMission do
		...
		if JobInProgress == true then
			...
			local dys3 = #(GetEntityCoords(GetPlayerPed(-1)) - vector3(loc.Location.x, loc.Location.y, loc.Location.z))
            if (dys3 > 60) and DeliveryInProgress == false then
				DrawMissionText("Podazaj ~y~do lokacji~s~ zaznaczonej na twoim GPS")
			end
			
			if (dys3 <= 25) and not JobVanSpawned then
				ClearAreaOfVehicles(loc.Location.x, loc.Location.y, loc.Location.z, 15.0, false, false, false, false, false) 
				missionCoords = {loc.Location.x, loc.Location.y, loc.Location.z}
				JobVanSpawned = true
                while ESX == nil do
                    Citizen.Wait(1)
                end
				ESX.Game.SpawnVehicle('rumpo3', missionCoords, loc.Heading, function(vehicle)
			...
			if (dys3 < 150) and not GoonsSpawned then
			...
			if DeliveryInProgress == false and (dys3 < 80) and (dys3 > 10) then
				DrawMissionText("~r~Zabij~s~ chlopow ktorzy chronia ~y~Vana~s~")
			end
			
			if (dys3 < 40) and not JobPlayer then
				JobPlayer = true
				SetPedRelationshipGroupHash(GetPlayerPed(-1), GetHashKey("PLAYER"))
				AddRelationshipGroup('MissionNPCs')
				local i = 0
			...
			if isVehicleLockPicked == false and (dys3 < 10) then
				DrawMissionText("Otworz drzwi samochodu i ukradnij ~y~Vana~s~")
			end
			
			local VanPosition = GetEntityCoords(JobVan) 
			local dys2 = #(GetEntityCoords(GetPlayerPed(-1)) - VanPosition)
			if (dys2 <= 2) and isVehicleLockPicked == false then
				DrawText3Ds(VanPosition.x, VanPosition.y, VanPosition.z, "Nacisnij ~g~[G]~s~ aby ~y~otworzyc drzwi do Vana~s~")
			...
			if IsPedInAnyVehicle(GetPlayerPed(-1), true) and isVehicleLockPicked == true then
				if dys2 < 5 then
					...
					if GetEntityModel(vehicle) == GetHashKey('rumpo3') then
						RemoveBlip(blip)
			...
			if DeliveryInProgress == true and isVehicleLockPicked == true then
				DrawMissionText("Dostarcz  ~y~vana~s~ do miejsca ~s~podrozy~s~ zaznaczonego na twoim GPS!")
			end
			...
			if DeliveryInProgress == true then
				...
				for k,v in pairs(Config.DeliveryPoints) do
					if(#(GetEntityCoords(GetPlayerPed(-1)) - vector3(v.x, v.y, v.z)) < Config.DeliveryDrawDistance) then
					...
					if(#(GetEntityCoords(GetPlayerPed(-1)) - vector3(v.x, v.y, v.z)) < 2.0) and vanIsDelivered == false then
						DrawText3Ds(v.x, v.y, v.z, "Nacisnij ~g~[E]~s~ aby ~y~Dostarczyc~s~")
						if IsControlJustPressed(0, 38) then 
			...
			if StopMission == true then
				...
				if Config.EnableCustomNotification == true then
					...
					ESX.ShowNotification("~r~Porazka:~s~ Zginales")
				end	
...
function LockpickVanDoor()
	...
	exports['MajorkaRP']:startUI(9500, "Otwieranie Zamka Samochodu")
	Citizen.Wait(9500)
	...
	ESX.ShowNotification("~g~Skutecznie~s~ ~r~otworzyles~s~  ~y~Vana~s~")
end
