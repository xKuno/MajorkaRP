	...
	while true do
		...
		for k,v in pairs(Config.GoldSmeltery) do
			...
			if distance <= 1.5 and meltingGold == false then
				DrawText3Ds(v.x, v.y, pos.z, "Nacisnij ~g~[E]~s~ aby ~y~Stopic Zloto~s~")
	...
	while true do
		...
		for k,v in pairs(Config.GoldExchange) do
			...
			if distance <= 0.6 then
				DrawText3Ds(v.x, v.y, pos.z, "Nacisnij ~g~[E]~s~ aby ~y~wymienic Zloto~s~")
				if IsControlJustPressed(0, 38) then
...
AddEventHandler("GoldWatchToGoldBar", function()
	...
	exports['MajorkaRP']:startUI((Config.SmelteryTime * 1000), "ZLOTE ZEGARKI -> SZTABKI ZLOTA")
...
AddEventHandler("GoldBarToCash", function()
	...
	--TaskStartScenarioInPlace(PlayerPedId(), "PROP_HUMAN_BUM_BIN", 0, true)
	exports['MajorkaRP']:startUI((Config.ExchangeTime * 1000), "WYMIANIA ZLOTYCH SZTABEK")
	...
	converting = false
	ESX.ShowNotification("Otrzymales ~r~$75.000~s~ Brudnych Pieniedzy")
end)
