...
RegisterCommand("wyscig", function(source, args)
    if args[1] == "wyczysc" or args[1] == "opusc" then
    ...
    elseif args[1] == "ustaw" then
        -- Clear waypoint, cleanup recording and set flag to start recording
        TriggerEvent('chatMessage', '[WYSCIGI ULICZNE]', {255, 0, 0}, "ustaw punkty na mapie")
        SetWaypointOff()
        cleanupRecording()
        ...
        if IsPedInAnyVehicle(player, false) then
            ...
            if raceStatus.state == RACE_STATE_RACING then
                ...
                if config_cl.hudEnabled then
                    ...
                    Draw2DText(config_cl.hudPosition.x, config_cl.hudPosition.y + 0.04, ("~y~CHECKPOINT %d/%d (%dm) | OKRAZENIE %d/%d | MIEJSCE %d/%d"):format(raceStatus.checkpoint, #race.checkpoints, checkpointDist, raceStatus.currentLap, race.laps, raceStatus.myPosition, raceStatus.totalPlayers), 0.5)
                end
            ...
            elseif raceStatus.state == RACE_STATE_JOINED then
                ...
                elseif count <= config_cl.freezeDuration then
                    ...
                    Draw3DText(race.startCoords.x, race.startCoords.y, zCoord+1.0, ("Wyscig o ~g~$%d~w~ rozpoczyna sie za ~y~%d~w~s"):format(race.amount, math.ceil(count/1000.0)))
                    Draw3DText(race.startCoords.x, race.startCoords.y, zCoord+0.80, "Dolaczono")
                    raceStatus.totalPlayers = raceStatus.totalPlayers+1
                end
            -- Player is not in a race
                ...
                for index, race in pairs(races) do
                    ...
                    if proximity < config_cl.joinProximity and currentTime < race.startTime then
                        ...
                        Draw3DText(race.startCoords.x, race.startCoords.y, zCoord+1.0, ("Wyscig o ~g~$%d~w~ rozpoczyna sie za ~y~%d~w~s"):format(race.amount, count))
                        Draw3DText(race.startCoords.x, race.startCoords.y, zCoord+0.80, "Wcisnij [~g~E~w~] aby dolaczyc")
...
TriggerEvent('chat:addSuggestion', '/wyscig start', 'Wystartuj wyscig', {
    { name="LAP", help="Ilosc okrazen" },
    { name="CZAS", help="Czas do startu" },
    { name="$", help="Stawka wyscigu" },  
})

TriggerEvent('chat:addSuggestion', '/wyscig', 'Menu wyscigow', {
    { name="Opcje", help="ustaw/zapisz/lista/zaladuj/start/usun/anuluj/wyczysc/opusc" }
})