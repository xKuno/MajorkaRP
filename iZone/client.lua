...
bimber = '~w~Idealne miejsce na wytworzenie ~g~bimbru'
zacier = '~w~Idealne miejsce na wytworzenie ~g~zacieru'
acetone = '~w~Idealne miejsce na wytworzenie ~g~Acetonu'
tekst = '~w~Ziemia w tym miejscu wygląda na wyjątkowo ~g~uprawna'
spalone = '~w~Dookola widac rozciagniete tasmy ~r~LSPD'

Citizen.CreateThread(function()
    Wait(2000)
	print('strefy zaladowane')
        while true do
            Citizen.Wait(0)
			if notinCar() then
				...
				for i = 1, #Zones do
					chuj = Zones[i].center
					local dystans = #(vector3(chuj.x, chuj.y, chuj.z) - coords)
					if dystans < 10 then
						if Zones[i].name == 'narko' then
						DrawText3Ds(chuj.x, chuj.y, chuj.z, tekst)
						end
						if Zones[i].name == 'spalone' then
						DrawText3Ds(chuj.x, chuj.y, chuj.z, spalone)
						end
						if Zones[i].name == 'bimber' then
							DrawText3Ds(chuj.x, chuj.y, chuj.z, bimber)
						end
						if Zones[i].name == 'zacier' then
							DrawText3Ds(chuj.x, chuj.y, chuj.z, zacier)
						end
						if Zones[i].name == 'acetone' then
							DrawText3Ds(chuj.x, chuj.y, chuj.z, acetone)
						end
						--DrawText3Ds(ix, iy, iz, EventPlace.text)
					end
				end
			else
				Citizen.Wait(1500)
...
RegisterNetEvent('destyler')
AddEventHandler('destyler', function()
	miejscowka = 'null'
	local coords = GetEntityCoords(GetPlayerPed(-1))
	for i = 1, #Zones do
		chuj = Zones[i].center
		...
		if dystans < 10 then
			miejscowka = 'cos'
			if Zones[i].cat == 'destyler' then
				miejscowka = Zones[i].name
				TriggerServerEvent("metka:strefa", miejscowka)
				--print(miejscowka)
				--DrawText3Ds(ix, iy, iz, EventPlace.text)
			else
				TriggerServerEvent("metka:strefa", 'chuj')
			end
		end
	end
	if miejscowka == 'null' then
		TriggerServerEvent("metka:strefa", 'chuj')
	end
end)