	...
	while true do
		...
		if IsEntityDead(PlayerPedId()) then
			...
			if (Killer == PlayerId()) then
				DeathReason = 'Popelnil samobojstwo'
	
			elseif (Killer == nil) then
				DeathReason = 'Umarl'
			else
				if IsMelee(DeathCauseHash) then
					DeathReason = 'Zamordowany'
				elseif IsTorch(DeathCauseHash) then
					DeathReason = 'Podpalony'
				elseif IsKnife(DeathCauseHash) then
					DeathReason = 'Zanozowany'
				elseif IsPistol(DeathCauseHash) then
					DeathReason = 'Zastrzelił pistoletem'
				elseif IsSub(DeathCauseHash) then
					DeathReason = 'Zastrzelił SMG'
				elseif IsRifle(DeathCauseHash) then
					DeathReason = 'Zastrzelił Karabinem'
				elseif IsLight(DeathCauseHash) then
					DeathReason = 'Zastrzelił Karabinem Maszynowym'
				elseif IsShotgun(DeathCauseHash) then
					DeathReason = 'Zastrzelił Shotgunem'
				elseif IsSniper(DeathCauseHash) then
					DeathReason = 'Zastrzelił Karabinem Snajperskim'
				elseif IsHeavy(DeathCauseHash) then
					DeathReason = 'Zastrzelił Ciezkim Karabinem'
				elseif IsMinigun(DeathCauseHash) then
					DeathReason = 'Zastrzelił Minigunem'
				elseif IsBomb(DeathCauseHash) then
					DeathReason = 'Rozerwał'
				elseif IsVeh(DeathCauseHash) then
					DeathReason = 'Przeciął'
				elseif IsVK(DeathCauseHash) then
					DeathReason = 'Rozjechał'
				else
					DeathReason = 'Zabił'
				end
			end
			
			if DeathReason == 'Popelnil samobojstwo' or DeathReason == 'Umarl' then
				TriggerServerEvent('DiscordBot:playerDied1', GetPlayerName(PlayerId()) .. ' ' .. DeathReason .. '.', Weapon)
			else
				TriggerServerEvent('DiscordBot:playerDied1', GetPlayerName(Killer) .. ' ' .. DeathReason .. ' ' .. GetPlayerName(PlayerId()) .. '.', Weapon)
			end
			Killer = nil
			DeathReason = nil
			DeathCauseHash = nil
