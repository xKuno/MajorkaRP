...
--End config
local chce_pow = false
local timing = timer * 60000 --Don't touche it

GetPlayerName()
RegisterNetEvent('outlawNotify')
AddEventHandler('outlawNotify', function(alert)
	if PlayerData.job ~= nil and (PlayerData.job.name == 'police' or PlayerData.job.name == 'IAA') then
...
RegisterNetEvent('esx_out:powon')
AddEventHandler('esx_out:powon', function()
	if PlayerData.job ~= nil and (PlayerData.job.name == 'police' or PlayerData.job.name == 'IAA') then
		if chce_pow == false then 
			chce_pow = true
				ESX.ShowNotification('Powiadomienia 505 ~g~ON')
		else
			chce_pow = false
				ESX.ShowNotification('Powiadomienia 505 ~r~OFF')
	    end
	else
		ESX.ShowNotification('Nie dla ciebie ta kaczka ')
	end
end)
...
AddEventHandler('gunshotPlace', function(gx, gy, gz, pjob)
	if PlayerData.job ~= nil and (PlayerData.job.name == 'police' or PlayerData.job.name == 'IAA') then
		if gunshotAlert then
			...
			if pjob == "police" or pjob == 'IAA' then
...
AddEventHandler('esx:wytrychalert', function()
		--if PlayerData.job.name ~= 'police' and PlayerData.job.name ~= 'ambulance' and PlayerData.job.name ~= 'mechanic'  then
			...
			if PlayerData.job ~= nil and (PlayerData.job.name == 'police' or PlayerData.job.name == 'IAA') and showcopsmisbehave == false then
			elseif PlayerData.job ~= nil and (PlayerData.job.name == 'police' or PlayerData.job.name == 'IAA') and showcopsmisbehave then
...
Citizen.CreateThread( function()
    while true do
        Wait(0)
            ...
            if IsPedShooting(GetPlayerPed(-1)) and not IsPedCurrentWeaponSilenced(GetPlayerPed(-1)) and GetSelectedPedWeapon(GetPlayerPed(-1)) ~= GetHashKey("weapon_stungun") and GetSelectedPedWeapon(GetPlayerPed(-1)) ~= GetHashKey("weapon_ball") and GetSelectedPedWeapon(GetPlayerPed(-1)) ~= GetHashKey("weapon_petrolcan") and GetSelectedPedWeapon(GetPlayerPed(-1)) ~= GetHashKey("weapon_fireextinguisher") and GetSelectedPedWeapon(GetPlayerPed(-1)) ~= GetHashKey("weapon_snowball") then
...
RegisterCommand('powiadomienia_505', function()
    Citizen.CreateThread(function()
	TriggerEvent('esx_out:powon')
	end)
end, false)