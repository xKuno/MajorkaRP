...
RegisterCommand('wozek', function()
	if ESX.PlayerData.job.name == 'ambulance' or ESX.PlayerData.job.name == 'police' then
		...
		ShowNotification("~r~Aby móc tego użyć musisz być medykiem!")
	end
end, false)

RegisterCommand('schowajwozek', function()
	local wheelchair = GetClosestObjectOfType(GetEntityCoords(PlayerPedId()), 15.0, GetHashKey('prop_wheelchair_01'))
	...
	while true do
		...
		if DoesEntityExist(closestObject) then
			...
			if #(pedCoords - pickupCoords) <= 1.0 then
				DrawText3Ds(pickupCoords, "[E] Przemieść wózek", 0.4)
...
Sit = function(wheelchairObject)
	...
	if closestPlayer ~= nil and closestPlayerDist <= 1.5 then
		if IsEntityPlayingAnim(GetPlayerPed(closestPlayer), 'missfinale_c2leadinoutfin_c_int', '_leadin_loop2_lester', 3) then
			ShowNotification("~r~Nie łap za nie swoje!")
			return
		end
	end
...
PickUp = function(wheelchairObject)
	...
	if closestPlayer ~= nil and closestPlayerDist <= 1.5 then
		if IsEntityPlayingAnim(GetPlayerPed(closestPlayer), 'anim@heists@box_carry@', 'idle', 3) then
			ShowNotification("~r~Nie siadaj komuś na kolana!")
			return
		end
	end
...
GetClosestPlayer = function()
	...
	for index,value in ipairs(players) do
		...
		if(target ~= ply) then
			...
			local distance = #(targetCoords - vector3(plyCoords["x"], plyCoords["y"], plyCoords["z"]))
