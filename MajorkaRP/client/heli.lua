...
Citizen.CreateThread(function()
	while true do
		...
		if (IsPlayerInPolmav() or IsPlayerInBuzzard()) and target_vehicle and not helicam and vehicle_display ~=2 then
...
function IsPlayerInPolmav()
	...
	return IsVehicleModel(vehicle, polmav_hash, m65c_hash, lguard_hash)
end

function IsPlayerInBuzzard()
	local lPed = GetPlayerPed(-1)
	local vehicle = GetVehiclePedIsIn(lPed)
	return IsVehicleModel(vehicle, buzzard_hash, m65c_hash, lguard_hash)
end
...
function RenderVehicleInfo(vehicle)
	if DoesEntityExist(vehicle) then
		...
		SetTextEntry("STRING")
		if vehicle_display == 0 then
			AddTextComponentString("Predkosc: " .. math.ceil(vehspeed) .. " " .. speed_measure .. "\nModel: " .. vehname .. "\nTablica: " .. licenseplate)
		elseif vehicle_display == 1 then
