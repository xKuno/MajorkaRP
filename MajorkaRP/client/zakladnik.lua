...
RegisterCommand("zakladnik",function()
	if beingHeldHostage then
		exports['mythic_notify']:DoCustomHudText('error', 'Nie możesz tego zrobić jesteś zakładnikiem!', 15000)
	else
		takeHostage()
	end
end)
...
function takeHostage()
	...
	if not canTakeHostage then 
		ESX.ShowNotification("Musisz mieć załadowany pistolet, aby móc wziąć zakładnika.")
	end
...
Citizen.CreateThread(function()
	while true do 
		if holdingHostage then
			...
			DrawText3D(playerCoords.x,playerCoords.y,playerCoords.z,"Nacisnij [G] aby puscic, [L] aby zabic")
			if IsDisabledControlJustPressed(0,47) then --release	
				...
				Wait(100)
				releaseHostage()
			elseif IsDisabledControlJustPressed(0,7) then --kill 			
				holdingHostage = false
				holdingHostageInProgress = false 		
				local closestPlayer = GetClosestPlayer(2)
				target = GetPlayerServerId(closestPlayer)
