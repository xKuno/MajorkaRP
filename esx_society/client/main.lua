...
function StanKonta(society)
	ESX.TriggerServerCallback('esx_society:getSocietyMoney', function(money)
		stan_society = tonumber(money)
	end, society)
end
...
function OpenBossMenu(society, close, options)
	...
	if options.balance then
		table.insert(elements, {label = '----------[Stan Konta]----------', value = 'cipa'})
		table.insert(elements, {label = '<span style="color:green;">'..math.floor(tonumber(stan_society))..'$', value = 'cipa'})
		table.insert(elements, {label = '----------[Stan Konta]----------', value = 'cipa'})
	end
	...
	if options.limit then
		table.insert(elements, {label = _U('count_management'), value = 'powieksz_limit'})
	end
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'boss_actions_' .. society,
	{
		...
		elseif data.current.value == 'powieksz_limit' then
			OpenPowiekszanieLimitu(society)
		elseif data.current.value == 'cipa' then
...
function OpenPowiekszanieLimitu(society)

	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'powieksz_limit_' .. society,
	{
		title    = _U('employee_management'),
		align    = 'center',
		elements = {
			{label = 'KOSZT POWIĘKSZENIA O 2 OSOBY TO OD 2 DO 5 MLN', value = 'cipa'},
			{label = 'Powiększ limit osób o 2', value = 'powieksz_limit'},
			{label = 'KOSZT POWIĘKSZENIA O 2 OSOBY TO OD 2 DO 5 MLN', value = 'cipa'}
		}
	}, function(data, menu)
		...
		if data.current.value == 'powieksz_limit' then
			ESX.TriggerServerCallback('esx_society:getpowiekszenie', function(zatrudnieni)
				if zatrudnieni < 50 then
					TriggerServerEvent("esx_society:powiekszsloty", society, 1)
				elseif zatrudnieni >= 50 and zatrudnieni < 75 then
					TriggerServerEvent("esx_society:powiekszsloty", society, 2)
				elseif zatrudnieni == 75 then
					TriggerServerEvent("esx_society:powiekszsloty", society, 3)
				end
			end, society)
		end
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'manage_employees_' .. society,
	{
		...
		if data.current.value == 'employee_list' then
			if ESX.PlayerData.job.name == society then
				if ESX.PlayerData.job.grade_name == 'boss' or ESX.PlayerData.job.grade_name == 'zastepca' then
		...
		if data.current.value == 'recruit' then
			ESX.TriggerServerCallback('esx_society:getliczbasociety', function(zatrudnieni)
				if zatrudnieni then
					if ESX.PlayerData.job.name == society then
...
function OpenEmployeeList(society)
	...
	ESX.TriggerServerCallback('esx_society:getEmployees', function(employees)
		...
		for _,i in pairs(employees) do
			local gradeLabel = (i.job.grade_label == '' and i.job.label or i.job.grade_label)
...
function OpenEmployeeList2(society)
  ...
  ESX.TriggerServerCallback('esx_society:getEmployees2', function(employees)
		...
		for _,i in pairs(employees) do
			...
			local gradeLabel = (i.job2.grade_label == '' and i.job2.label or i.job2.grade_label)
...
function OpenPromoteMenu(society, employee)
	...
	ESX.TriggerServerCallback('esx_society:getJob', function(job)
		...
		for _,i in pairs(job.grades) do
			local gradeLabel = (i.label == '' and job.label or i.label)

			table.insert(elements, {
				label = gradeLabel,
				value = i.grade,
				selected = (employee.job.grade == i.grade)
			})
		end
...
function OpenPromoteMenu2(society, employee)
  ...
  ESX.TriggerServerCallback('esx_society:getJob2', function(job2)
		...
		for _,i in pairs(job2.grades) do
			local gradeLabel = (i.label == '' and job2.label or i.label)

			table.insert(elements, {
				label = gradeLabel,
				value = i.grade,
				selected = (employee.job2.grade == i.grade)
			})
		end
