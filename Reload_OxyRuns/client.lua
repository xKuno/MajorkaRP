...
local pillWorker = { ['x'] = 69.02,['y'] = -1569.87,['z'] = 29.6,['h'] = 230.04, ['info'] = 'boop bap' }
...
local drugLocs = { -- nieuzywane
	[1] =  { ['x'] = 1313123.94,['y'] = -1937.95,['z'] = 20.61,['h'] = 118.59, ['info'] = ' Grove Stash' },
	[2] =  { ['x'] = 139130.84,['y'] = -1507.94,['z'] = 58.44,['h'] = 29.6, ['info'] = ' East Side' },
	[3] =  { ['x'] = -6731236.81,['y'] = -877.94,['z'] = 24.48,['h'] = 324.9, ['info'] = ' Wei Cheng' },
}
...
--[[function CreateOxyVehicle()
    ...
    SetVehicleHasBeenOwnedByPlayer(oxyVehicle,true)
	 ...
	 while true do
    	Citizen.Wait(10)
    	 DrawText3Ds(carspawns[spawnpoint]["x"], carspawns[spawnpoint]["y"], carspawns[spawnpoint]["z"], "~g~Samochód przeznaczony do Pracy!~r~(Praca innym samochodem jest niezgodna z Konstytucją).")
...
function DoDropOff()
	local success = true
	local sprzedawanie = true 
	local OxyChance = math.random(1,1000)
	TriggerEvent('oxydelivery:freezePlayer', true)
	Citizen.Wait(1000)
	playerAnim()
	Citizen.Wait(800)
	...
	if DoesEntityExist(deliveryPed) and not IsEntityDead(deliveryPed) then
		...
		if success then
			...
			Citizen.Wait(2000)
			TriggerEvent("pNotify:SendNotification",{text = "<b style = color:aqua>Kolejne miejsce dostawy zostanie za moment zaznaczone na GPS</b>"})
		elseif salecount == Config.RunAmount then   
		TriggerEvent("pNotify:SendNotification",{text = "<b style = color:blue>Dostraczyłeś wszystkie paczki</b>",2}) 	
		else
		TriggerEvent("pNotify:SendNotification",{text = "<b style = color:red>Dostawa nieudana</b>",2})
	end 
		TriggerEvent('oxydelivery:freezePlayer', false)
		DeleteCreatedPed()
	end
end 
end
...
AddEventHandler("oxydelivery:client", function()
	...
	while tasking do
		...
		if dstcheck < 40.0 and not pedCreated then
			...
			DeleteCreatedPed()
			CreateOxyPed()
			TriggerEvent("pNotify:SendNotification",{text = "<b style = color:green>Jestes blisko miejsca dostawy</b>"})
		end
		toolong = toolong - 1
		if toolong < 0 then
		   ...
		   -- SetVehicleHasBeenOwnedByPlayer(oxyVehicle,false)
			...
			tasking = false
			OxyRun = false
			TriggerEvent("pNotify:SendNotification",{text ="<b style =purple>Twoje zlecenie zostalo anulowane, ze wzgledu na zbyt dlugi czas oczekiwania</b>", 2})
		end
...
Citizen.CreateThread(function()
    while true do
		...
		if dropOff6 < 2 and not OxyRun then
			
			--DrawText3Ds(pillWorker["x"],pillWorker["y"],pillWorker["z"], "[E]  $500- Praca Dorywcza") 
...
Citizen.CreateThread(function()
    ...
    while true do
		...
		elseif OxyRun then
			...
			--if not DoesEntityExist(oxyVehicle) or GetVehicleEngineHealth(oxyVehicle) < 5.0 or GetVehicleBodyHealth(oxyVehicle) < 5.0 then
				--OxyRun = false
				--tasking = false
				-- TriggerEvent("pNotify:SendNotification", {text="Nie możesz wykonać więcej zleceń z względu na stan twojego samochodu", 2})
			--else
				if tasking then
			        Citizen.Wait(5000)
			    else
				    ...
				    if salecount == Config.RunAmount then
				    	TriggerEvent("pNotify:SendNotification", {text ="<b style = color:white>Następne zlecenie, będzie ostatnim zleceniem tego kursu.</b>"})
				    	Citizen.Wait(600)
				    	OxyRun = false
				    end
				end
				...
				if dstcheck < 5.0 then
					...
					if job ~= "police" then
				    	...
				    	if IsControlJustReleased(0,38) then
							...
							if not daytime then
		            			TriggerEvent("pNotify:SendNotification",{text ="Its too late - noones buying shit!",2})
		            		else
...
AddEventHandler("oxydelivery:startDealing", function()
	...
	OxyRun = true
	firstdeal = true
	TriggerEvent("pNotify:SendNotification",{text = "Za moment zostanie oznaczony cel na GPS<b style =color:white>(Biała Paczka)</b>, jedź do niego <b style = color:green>Prywatnym Samochodem.</b>"})
	--CreateOxyVehicle()
end)

RegisterNetEvent('oxydelivery:freezePlayer')
AddEventHandler('oxydelivery:freezePlayer', function(freeze)
