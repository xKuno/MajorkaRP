    ...
    Citizen.CreateThread(function()
        ...
        while true do
            ...
            for location, val in pairs(Config.Teleporters) do
				...
				local JobNeeded2 = val['Job']

                local dstCheckEnter, dstCheckExit = #(plyCoords - vector3(Enter['x'], Enter['y'], Enter['z'])), #(plyCoords - vector3(Exit['x'], Exit['y'], Exit['z']))

                if dstCheckEnter <= 7.5 then
                    if JobNeeded ~= 'none' then
                        if PlayerData.job.name == JobNeeded or PlayerData.job2.name == JobNeeded2 then
