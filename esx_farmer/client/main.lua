...
function OpenCloakroom()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'farmer_cloakroom',
	{
		...
		onDuty = false
		offduty()
		exports.pNotify:SendNotification({text = "Koniec pracy na dziś!", type = "info", queue = "global", timeout = 10000, layout = "centerLeft"})
		menu.close()
		farmerblip()
		...
		onduty()
		onDuty = true
		exports.pNotify:SendNotification({text = "Rozpoczales prace!", type = "info", queue = "global", timeout = 10000, layout = "centerLeft"})
		Citizen.Wait(3000)
		exports.pNotify:SendNotification({text = "Pod komendą /farmer masz przydatne informacje na temat pracy!", type = "info", queue = "global", timeout = 10000, layout = "centerLeft"})
				menu.close()
...
function OpenSell()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'farmer_sell',
	{
		...
		exports.pNotify:SendNotification({text = "Gdzie auto z farmy?", type = "info", queue = "global", timeout = 10000, layout = "centerRight"})
	...
	while true do
		...
		if ESX.PlayerData.job and ESX.PlayerData.job.name == 'farmer' then
			...
			for k,v in pairs(Config.Zones) do
				...
				local distance2 = #(coords - vector3(Config.Zones.Cloakroom.Pos.x, Config.Zones.Cloakroom.Pos.y, Config.Zones.Cloakroom.Pos.z))
				
				if v.Type ~= -1 and distance < Config.DrawDistance and onDuty == true then
					...
					DrawText3Ds(Config.Zones.Cloakroom.Pos.x, Config.Zones.Cloakroom.Pos.y, Config.Zones.Cloakroom.Pos.z+0.5, 'Nacisnij ~b~[E]')
					end
				end
...
AddEventHandler('farmer:limit', function()
	zbieranie = false
	exports.pNotify:SendNotification({text = "Osiągnięto limit.", type = "info", queue = "global", timeout = 5000, layout = "centerRight"})
end)
...
AddEventHandler('farmer:maszchleb', function()
		exports.pNotify:SendNotification({text = "Towar z ostatnich zbiorów nie został sprzedany.", type = "info", queue = "global", timeout = 2000, layout = "centerRight"})
end)

RegisterNetEvent('farmer:empty')
AddEventHandler('farmer:empty', function()
		exports.pNotify:SendNotification({text = "Chyba nie masz tego czego potrzebuję...", type = "info", queue = "global", timeout = 2000, layout = "centerRight"})
end)
...
function onduty()
  ...
  local blipFarmer3 = AddBlipForCoord(Config.Zones.VehicleSpawner.Pos.x, Config.Zones.VehicleSpawner.Pos.y, Config.Zones.VehicleSpawner.Pos.z)
  ...
  table.insert(JobBlips, blipFarmer2)
    ...
    BeginTextCommandSetBlipName("STRING")
  AddTextComponentSubstringPlayerName('Garaz farmy')
...
RegisterCommand("farmer", function(source, args, raw)
              farmerhelp()
end, false)
