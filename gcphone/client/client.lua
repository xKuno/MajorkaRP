...
Citizen.CreateThread(function()
  while true do
    Citizen.Wait(0)
  if IsPedInAnyVehicle(GetPlayerPed(-1), false) or menuIsOpen == true then
    DisplayRadar(true)  
  else
    DisplayRadar(false)
    end
...
AddEventHandler("gcPhone:receiveMessage", function(message, sourcePlayer)
  SendNUIMessage({event = 'newMessage', message = message})
if message.owner == 0 then
     for _,contact in pairs(contacts) do
        if contact.number == message.transmitter then
      local pid2 = GetPlayerFromServerId(sourcePlayer)
      local mugshot, mugshotStr = ESX.Game.GetPedMugshot(GetPlayerPed(pid2))
      ESX.ShowAdvancedNotification("#" .. message.transmitter, contact.display, message.message, mugshotStr, 1)
  ...
  UnregisterPedheadshot(mugshot)   
    ...
    if not sourceKnown then
      pic = 'CHAR_HUMANDEFAULT'
      ESX.ShowAdvancedNotification('Wiadomość', message.transmitter, '~y~' .. message.message, pic, 1)
    end
