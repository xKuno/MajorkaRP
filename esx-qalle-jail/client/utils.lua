	...
	TriggerEvent('skinchanger:getSkin', function(skin)
		if skin.sex == 0 then
			local clothesSkinek = {
				...
				['torso_1'] = 5, ['torso_2'] = 5,
				['arms'] = 5,
				['pants_1'] = 2, ['pants_2'] = 11,
				['shoes_1'] = 1, ['shoes_2'] = 1,
			}
			TriggerEvent('skinchanger:loadClothes', skin, clothesSkinek)
...
function TeleportPlayer(pos)
	...
	if #Values["goal"] > 1 then
		...
		function(data, menu)
			...
			if action == "Ochrona" or action == "Wiezienie" then

				if PlayerData.job.name ~= "police" then
					ESX.ShowNotification("Nie masz klucza aby sie tam dostac!")
					return
				end
			end
