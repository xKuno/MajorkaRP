...
Citizen.CreateThread(function()
	while true do
		...
		local Dystans = GetDistanceBetweenCoords(PozycjaGracza, 948.85, 33.49, 71.84, true)
		local Dystans2 = GetDistanceBetweenCoords(PozycjaGracza, 959.5, 52.74, 74.99, true)
		local Dystans3 = GetDistanceBetweenCoords(PozycjaGracza, 965.17, 48.62, 74.99, true)
		if Dystans <= 10.0 then
			...
			ESX.Game.Utils.DrawText3D(PozycjaTekstu, "Wcisnij [~g~E~s~] by kupic zetony", 0.55, 1.5, "~b~Kasjer", 0.7)
		...
		if Dystans2 <= 6.0 then
			...
			ESX.Game.Utils.DrawText3D(PozycjaTekstu2, "Wcisnij [~g~E~s~] by otworzyc bar", 0.55, 1.5, "~b~BAR", 0.7)
		...
		if Dystans3 <= 6.0 then
			...
			ESX.Game.Utils.DrawText3D(PozycjaTekstu3, "Wcisnij [~g~E~s~] by otworzyc bar", 0.55, 1.5, "~b~BAR", 0.7)
	  ...
	  function(data, menu)
		...
		if akcja == 'buy' then
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'get_item_count', {
				title = 'Koszt - $15/zeton'
			}, function(data2, menu2)

				local quantity = tonumber(data2.value)

				if quantity == nil then
					TriggerEvent("pNotify:SendNotification", {text = 'Niepoprawna kwota!'})
				else
	...
	ESX.UI.Menu.Open(
      'default', GetCurrentResourceName(), 'alkohole',
      {
          ...
          elements = {
			...
			{label = "Wodkaa", value = "vodka"},
		  }
      },
	  function(data, menu)
		local item = data.current.value
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'buy_alkohol', {
				title = 'Koszt - $10/rzecz'
			}, function(data2, menu2)

				local quantity = tonumber(data2.value)

				if quantity == nil then
					TriggerEvent("pNotify:SendNotification", {text = 'Niepoprawna ilosc!'})
				else
...
RegisterNUICallback('wygrana', function(data)
	TriggerEvent('pNotify:SendNotification', {text = 'Wygrales '..data.win..' zetonow!'})
end)
...
Citizen.CreateThread(function()
	while true do
		...
		for i=1, #Config.Sloty do
			...
			if dis <= 3.0 then
				ESX.ShowHelpNotification('Wcisnij ~INPUT_PICKUP~ by grac.')
