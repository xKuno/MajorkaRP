...
RegisterNUICallback('chatResult', function(data, cb)
  ...
  SetNuiFocus(false)
  TriggerServerEvent('3dme:playertyping', nil)
...
RegisterNetEvent('3dme:playertyping')
AddEventHandler('3dme:playertyping', function(text, source)
    local offset = 1 + (nbrDisplaying*0.10)
    opis_tekst[source] = nil
    local player = GetPlayerFromServerId(source)
    if chatInputActive and player ~= -1 then
        opis_tekst[source] = text
        local ped = GetPlayerPed(player)
        Displaykropka(ped, text, offset, source)
    end
end)

function Displaykropka(mePlayer, text, offset, source)
    Citizen.CreateThread(function()
        ...
        while opis_tekst[source] do
            ...
            if dist < 5 then
                ...
                z = z + offset
                DrawText3DOpis(x,y,z, vector3(x,y,z),"...", typ, {0.003, 0.02, 325})
                --print(x,y,z)
            end
        end
    end)
  ...
  while true do
    ...
    if not chatInputActive then
      if IsControlPressed(0, 245) --[[ INPUT_MP_TEXT_CHAT_ALL ]] then
        ...
        TriggerServerEvent('3dme:playertyping', "...")
        SendNUIMessage({
          type = 'ON_OPEN'
        })
      end
...
function DrawText3DOpis(x,y,z, gowno, text, typ, rect)
    ...
    local onScreen, _x, _y = World3dToScreen2d(x, y, z+0.15)
    DrawText(_x,_y)
