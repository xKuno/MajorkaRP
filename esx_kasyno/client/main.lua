	...
	ESX.UI.Menu.Open(
		...
		function(data, menu)
			...
			if data.current.value == 'job_wear' then
				...
				TriggerServerEvent('esx_kasyno:zaplac')
				end)
...
function MenuVehicleSpawner()
	...
	for _,i in pairs(Config.Trucks) do
		table.insert(elements, {label = GetLabelText(GetDisplayNameFromVehicleModel(i)), value = i})
	end
	...
	ESX.UI.Menu.Open(
		...
		function(data, menu)
			ESX.Game.SpawnVehicle(data.current.value, Config.Zones.VehicleSpawnPoint.Pos, 49.8, function(vehicle)
				...
				if data.current.value == 'phantom3' then
					ESX.Game.SpawnVehicle("trailers2", Config.Zones.VehicleSpawnPoint.Pos, 49.8, function(trailer)
...
function IsATruck()
	...
	for _,i in pairs(Config.Trucks) do
		if IsVehicleModel(GetVehiclePedIsUsing(playerPed), i) then
			isATruck = true
			break
		end
	end
...
AddEventHandler('esx_godirtyjob:hasEnteredMarker', function(zone)
	...
	if zone == namezone then
		if isInService and MissionLivraison and MissionNum == namezonenum and MissionRegion == namezoneregion  then
...
function nouvelledestination()
	...
	if livraisonnombre >= Config.MaxDelivery then
		...
		livraisonsuite = math.random(5, 100)
